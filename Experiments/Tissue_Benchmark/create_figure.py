...
for funcname in psi_names:
    shear_fit_func = pd.read_csv(os.path.join('plot_data/', 'data_shear_dokos_%s.csv' % (funcname))).drop(columns = "Unnamed: 0")
...
for funcname in psi_names:
    shear_fit_func_sommer = pd.read_csv(os.path.join('plot_data/', 'data_shear_sommer_%s.csv' % (funcname))).drop(columns = "Unnamed: 0")
...
height_ratios_top = [1, 1, 1, 1, 1.3, 1.3]
...
for axs in [sommer_params_axs, dokos_params_axs]:
    ...
    for i in range(2):
        axs[4 + i].set_yticks([0] + list(10 ** np.arange(1, 23, 4)))
...
for i, funcname in enumerate(psi_names):
    ...
    num_print = '%0.1E' % sRSS
    if funcname == "PZL":
        ax.annotate(r'$\boldsymbol{f_\mathbf{GoF} = }$' + '\n' + r'$\boldsymbol{%s \times 10^{-%s}}$' % (
        num_print[:3], num_print[-1]), (0.05, 0.92), xycoords="axes fraction", size=fs, va='top')
    else:
        ax.annotate(r'$f_\mathrm{GoF} = $' + '\n' + r'$%s \times 10^{-%s}$' % (num_print[:3], num_print[-1]),
                    (0.05, 0.92),
